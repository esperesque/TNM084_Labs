...
// 2023-05-09: Added timer removal.
...
void glutInit(int *argcp, char **argv)
{
	...
	hCrt = _open_osfhandle(
		(long) GetStdHandle(STD_OUTPUT_HANDLE), // This gives a warning but it works.
		_O_TEXT
		);
	hf = _fdopen( hCrt, "w" );
	*stdout = *hf;
...
// Added by Ingemar
void glutRepeatingTimerOLD(int millis)
{
	TimerRec *t	= (TimerRec *)malloc(sizeof(TimerRec));
	t->arg = 0;
	t->time = millis + glutGet(GLUT_ELAPSED_TIME);
...
// Added by Ingemar
// 2023: Added timer removal
void glutRepeatingTimer(int millis)
{
	if (millis <= 0) // Stop all timers.
	{
		TimerRec *t = gTimers;
		while (t)
		{
