...
//	char val;
	...
	{
	    // fx ranges from around [0, 0.25]
		fx = (float)(x-kTextureSize/2.)/kTextureSize*2.;
		fy = (float)(y-kTextureSize/2.)/kTextureSize*2.;

		// Move the origin to the lower right quadrant
		fx += 0.5;
		fy -= 0.5;

		fxo = sqrt(fx*fx+fy*fy);
		fyo = sqrt(fx*fx+fy*fy);
		fxo = cos(fxo * ringDensity * 0.48);
		fyo = sin(fyo * ringDensity * 2.66);
		if (fxo > 1.0) fxo = 1;
		if (fxo < -1.0) fxo = -1.0;
		if (fyo > 1.0) fyo = 1.0;
		if (fyo < -1.0) fyo = -1.0;
		ptex[x][y][0] = 64 + fxo * 64;
		ptex[x][y][1] = 128 + fyo * 64;
		ptex[x][y][2] = 64 + 64 * fyo * fxo;
	}
}
